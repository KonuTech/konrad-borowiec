      ...
      setTimeout(() => {
        ...
        if (descriptionElement) {
          ...
          const calculatedHeight = Math.max(
            isMobile ? 8 : 3,
            (descriptionHeight / 16) * multiplier,
          );
          document.documentElement.style.setProperty(
            '--timeline-line-height-last',
            `calc(100% + ${calculatedHeight}rem)`,
          );
        }
      }, 10);
    }
      ...
      <div className="flex items-start">
        <div className="relative mr-6">
          <div className="flex flex-col items-center">
            {/* Icon container */}
            <div
              className={`relative z-10 flex h-12 w-12 cursor-pointer items-center justify-center rounded-full ${
                item.type === 'work'
            ...
            <div className="relative min-h-[4rem] w-full">
              {!isLast && (
                <motion.div
                  className="absolute left-1/2 w-1 -translate-x-1/2 transform bg-portfolio-lighter dark:bg-[#4A90E2]"
                  initial={{ height: '4rem' }}
                  animate={{
                    ...
                    if (isActive) {
                      ...
                      setTimeout(() => {
                        const descriptionHeight =
                          document.querySelector(`.timeline-description-id-${index}`)
                            ?.clientHeight || 0;
                        document.documentElement.style.setProperty(
                          '--timeline-line-height',
                          `calc(100% + ${Math.max(4, descriptionHeight / 16)}rem)`,
                        );
                      }, 10);
                    }
                  }}
              ...
              {isLast && (
                <motion.div
                  ...
                  animate={{
                    height: isActive
                      ? 'var(--timeline-line-height-last, calc(100% + 6rem))'
                      : '4rem',
                  }}
                  transition={{
                    duration: 0.5,
