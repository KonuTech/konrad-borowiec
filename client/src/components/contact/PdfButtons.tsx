...
<body>
  ...
  const getMarkdownContent = (lang: 'en' | 'pl'): string => {
    const headerSubtitle =
      lang === 'en'
        ? 'Data Engineer | Financial Services Specialist'
        : 'Inżynier danych | Specjalista usług finansowych';
    ...
    const contactInfo =
      lang === 'en'
        ? [
            ...
            '- **LinkedIn:** [linkedin.com/in/32167](https://linkedin.com/in/32167)',
            '- **Credly:** [credly.com/users/konrad-borowiec/badges](https://credly.com/users/konrad-borowiec/badges)',
          ]
        : [
    ...
    return `# Konrad Borowiec\n\n${headerSubtitle}\n\n${contactInfo.join('\n')}\n\n## ${
      lang === 'en' ? 'About' : 'O Mnie'
    }\n\n${aboutParagraphs.map((p) => `- ${p}`).join('\n')}\n\n---\n\n## ${
      lang === 'en' ? 'Education' : 'Edukacja'
    }\n\n${educationItems.map((item) => `${item.degree}\n\n${item.institution}\n\n${item.dates}\n\n${item.description}`).join('\n\n')}\n\n---\n\n## ${
      lang === 'en' ? 'Experience' : 'Doświadczenie'
    }\n\n${experienceItems.map((item) => `${item.title}\n\n${item.company}\n\n${item.period}\n\n${item.tech}\n\n${item.description}\n\n- ${item.bullets.map((b) => b).join('\n')}`).join('\n\n')}\n\n---\n\n## ${
      lang === 'en' ? 'Side Projects' : 'Projekty poboczne'
    }\n\n${projects.map((project) => `${project.title}\n\n${project.tech}\n\n${project.description}`).join('\n\n')}\n\n---\n\n${footerText}`;
  };

  const generateMarkdownDownload = () => {
    const cvContent = getMarkdownContent(language);

    const link = document.createElement('a');
    link.href = 'data:text/markdown;charset=utf-8,' + encodeURIComponent(cvContent);
    link.download = `cv-konrad-borowiec-${language}-${currentDate}.md`;
    link.click();
  };
  ...
  return (
    <div className="flex justify-center gap-4">
      ...
      <button
        onClick={generateMarkdownDownload}
        className="flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-md transition-colors duration-300 hover:bg-portfolio-primary/10 dark:bg-portfolio-dark dark:hover:bg-portfolio-primary/30"
        aria-label="Download as Markdown"
        title="Download Markdown version"
      >
        <i className="fas fa-file-alt text-xl text-portfolio-primary dark:text-portfolio-lighter"></i>
      </button>
      <button
